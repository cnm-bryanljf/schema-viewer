...
`:
  ...
  gatherInvalid() {
    let t;
    const n = [], r = [];
    for (t = 0; t < this.tokens.length && Ar(this.tokens[t]); t += 1)
...
class hl {
    ...
    this.current = 0, this.errors = [], this.contextStack = new cl(), this.synchronizeProgram = () => {
      ...
      if (!this.check(f.RBRACE))
        for (g(this.advance()); !this.isAtEnd(); ) {
          const r = this.peek();
          if (this.check(f.RBRACE) || xn(this.previous(), r))
            break;
          g(r), this.advance();
        }
  ...
  shouldStopExpression() {
    if (this.isAtEnd() || Er(this.previous()))
      return !0;
  ...
  expression_bp(t) {
    ...
    for (; !this.isAtEnd(); ) {
      ...
      if (r.kind === f.LPAREN) {
        ...
        xn(this.previous(), r) && !this.contextStack.isWithinGroupExpressionContext() && !this.contextStack.isWithinListExpressionContext())
          break;
        try {
...
function ct(e) {
  ...
  if (e instanceof Ee && e.elementList.every(Un)) {
...
function bl(e) {
  var t;
  return !(e instanceof de) || !vr((t = e.op) == null ? void 0 : t.value) ? !1 : Je(e.leftExpression).and_then(() => Je(e.rightExpression)).unwrap_or(void 0) !== void 0;
}
function ml(e) {
...
class gl {
  ...
  validateName(t) {
    return t ? Lr(t) ? [] : [new c(l.INVALID_NAME, "An Enum name must be of the form <enum> or <schema>.<enum>", t)] : [new c(l.NAME_NOT_FOUND, "An Enum must have a name", this.declarationNode)];
  }
  validateAlias(t) {
...
class wl {
  ...
  validateName(t) {
    return t ? t instanceof De ? [new c(l.INVALID_NAME, "Invalid array as Table name, maybe you forget to add a space between the name and the setting list?", t)] : Lr(t) ? [] : [new c(l.INVALID_NAME, "A Table name must be of the form <table> or <schema>.<table>", t)] : [new c(l.NAME_NOT_FOUND, "A Table must have a name", this.declarationNode)];
  }
  validateAlias(t) {
  ...
  registerElement() {
    const t = [];
    this.declarationNode.symbol = this.symbolFactory.create(gr, { declaration: this.declarationNode, symbolTable: new Xe() });
  ...
  registerInjection(t) {
    ...
    const n = (p = t.partial.variable) == null ? void 0 : p.value, r = mr(n), s = this.symbolFactory.create(Zo, { declaration: t });
    t.symbol = s;
  ...
  validateFields(t) {
    return t.flatMap((n) => {
      if (!n.callee)
        return [];
      const r = [];
      n.args.length === 0 && r.push(new c(l.INVALID_COLUMN, "A column must have a type", n.callee)), X(n.callee) || r.push(new c(l.INVALID_COLUMN_NAME, "A column name must be an identifier or a quoted identifier", n.callee)), n.args[0] && !Sr(n.args[0]) && r.push(new c(l.INVALID_COLUMN_TYPE, "Invalid column type", n.args[0]));
  ...
  validateFieldSetting(t) {
    ...
    return a.length >= 1 && o.length >= 1 && s.push(
      ...
      switch (p) {
        ...
        case N.Ref:
          u.forEach((h) => {
            wr(h.value) || s.push(new c(l.INVALID_COLUMN_SETTING_VALUE, "'ref' must be a valid unary relationship", h.value || h.name));
          });
          break;
        case N.PKey:
  ...
  validateFieldSetting(t) {
    ...
    return o.length >= 1 && u.length >= 1 && i.push(...[...o, ...u].map((p) => new c(
      ...
      switch (h) {
        ...
        case N.Ref:
          p.forEach((d) => {
            wr(d.value) || i.push(new c(l.INVALID_COLUMN_SETTING_VALUE, `'${h}' must be a valid unary relationship`, d.value || d.name));
          });
          break;
        case N.PKey:
          p.length > 1 && i.push(...p.map((d) => new c(l.DUPLICATE_COLUMN_SETTING, `'${h}' can only appear once`, d))), p.forEach((d) => {
...
function _t(e, t, n) {
  ...
  for (const i of e) {
    ...
    if (r.has(o)) {
      ...
      a = new Xe();
      const u = n.create(Ir, { symbolTable: a });
      r.set(o, u);
    }
    r = a;
  }
...
function wr(e) {
  var n;
  if (!(e instanceof ne) || !vr((n = e.op) == null ? void 0 : n.value))
    return !1;
...
class Fn {
  constructor(t, n) {
    this.ast = t, this.symbolFactory = n, this.publicSchemaSymbol = this.symbolFactory.create(Ir, {
      symbolTable: new Xe()
...
class _e {
  ...
  bindFragments(t, n) {
    if (t.length === 0) return;
    const r = t.length === 1 && t[0] instanceof xe, s = [...n.topSubnamesSymbolKind], { remainingSubnamesSymbolKind: i } = n, a = r ? -1 : t.findIndex((E) => !X(E)), u = a !== -1 && Pr(t[a]) ? t[a] : void 0, p = a >= 0 ? s.pop() : void 0, h = t.slice(
      0,
      a === -1 ? void 0 : a
    );
  ...
  resolveIndexStack(t, n) {
    ...
    const [r, ...s] = t, i = Mn(r.index, this.declarationNode);
    if (i === void 0) {
  ...
  resolveInjections(t) {
    ...
    n.forEach((i, a) => {
      var p, h;
      if (!Wo(a)) return;
      const o = Mn(Ho(a), this.declarationNode), u = (h = (p = o == null ? void 0 : o.declaration) == null ? void 0 : p.symbol) == null ? void 0 : h.symbolTable;
      u && u.forEach((d, E) => {
...
class $l {
  ...
  resolve(t) {
    return this.ast.body.map((n) => {
      ...
      const r = Or(n), s = new r(n, this.errors);
...
class Xl {
  ...
  analyze() {
    return new Fn(this.ast, this.symbolFactory).validate().chain((n) => new $l(n).resolve(this.symbolFactory));
  }
  // For invoking the validator only
  validate() {
    return new Fn(this.ast, this.symbolFactory).validate().chain((n) => new ee(n, []));
  }
}
function Bn(e, t) {
...
function kr(e) {
  let t = null;
  e instanceof re && (t = e.argumentList.elementList.map((i) => ke(i) ? _r(i) : q(i) ? V(i).unwrap() : z(i).unwrap()).join(","), e = e.callee);
  let n = "";
  for (; e instanceof De; )
...
class Ur {
  ...
  interpretColumn(t) {
    ...
    if (F(i) instanceof B) {
      ...
      r.pk = !!((a = I[N.PK]) != null && a.length) || !!((o = I[N.PKey]) != null && o.length), r.increment = !!((u = I[N.Increment]) != null && u.length), r.unique = !!((p = I[N.Unique]) != null && p.length), r.not_null = (h = I[N.NotNull]) != null && h.length ? !0 : (d = I[N.Null]) != null && d.length ? !1 : void 0, r.dbdefault = Rr((b = (E = I[N.Default]) == null ? void 0 : E.at(0)) == null ? void 0 : b.value);
...
class Wl {
  ...
  interpretNote(t) {
    const n = ar(t, "callee.expression.literal.value", "");
...
function jr(e, t) {
  ...
  if (!((r == null ? void 0 : r.symbol) instanceof gr))
    return C();
...
const ic = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  DBMLCompletionItemProvider: Vr,
  DBMLDefinitionProvider: Xr,
  DBMLReferencesProvider: Wr
...
` && (u.pop(), p = !0);
  ...
  initMonacoServices() {
    return {
      definitionProvider: new Xr(this),
      referenceProvider: new Wr(this),
      autocompletionProvider: new Vr(this)
    };
  }
}
export {
